# -*- coding: utf-8 -*-
"""GA4 자동화 이메일 11.24 ver (Organic + 9 KPI Cards)
...
- 01 섹션: KPI 9개 카드 (3 x 3, Organic 포함)
- 02 섹션: 퍼널/채널/상품/검색 카드 (2 x 4 그리드)
- 시간대별 트래픽 & 매출: 섹션 2 아래 풀폭 카드로 트래픽 + 매출 시각화.
"""
...
def src_organic_uv_one_day(start_date_str: str, end_date_str: str) -> int:
    """
    특정 기간의 'Organic Search' 채널 UV만 합산해서 리턴.
    GA4 sessionDefaultChannelGroup 기준.
    """
    df = ga_run_report(
    ...
    return safe_int(df["sessions"].iloc[0])
...
def build_core_kpi():
    ...
    aov_prevday = (rev_prevday / ord_prevday) if ord_prevday else 0.0
    aov_prev = (rev_prev / ord_prev) if ord_prev else 0.0
    aov_yoy = (rev_yoy / ord_yoy) if ord_yoy else 0.0

    # --- Organic UV (어제/LW/LY) ---
    organic_today = src_organic_uv_one_day("yesterday", "yesterday")
    organic_prev = src_organic_uv_one_day("8daysAgo", "8daysAgo")
    organic_yoy = src_organic_uv_one_day("366daysAgo", "366daysAgo")

    organic_share_today = (organic_today / uv_today * 100) if uv_today else 0.0
    organic_share_prev = (organic_prev / uv_prev * 100) if uv_prev else 0.0
    organic_share_yoy = (organic_yoy / uv_yoy * 100) if uv_yoy else 0.0

    # 비오가닉 UV
    nonorganic_today = max(uv_today - organic_today, 0)
    nonorganic_prev = max(uv_prev - organic_prev, 0)
    nonorganic_yoy = max(uv_yoy - organic_yoy, 0)

    nonorganic_share_today = (nonorganic_today / uv_today * 100) if uv_today else 0.0
    ...
    kpi = {
        ...
        "revenue_prevday": rev_prevday,
        "revenue_prev": rev_prev,
        "revenue_yoy": rev_yoy,
        "revenue_ld_pct": pct_change(rev_today, rev_prevday),  # L.D
        "revenue_lw_pct": pct_change(rev_today, rev_prev),      # LW
        "revenue_ly_pct": pct_change(rev_today, rev_yoy),       # LY

        # --------------------
        # UV
        # --------------------
        "uv_today": uv_today,
        "uv_prevday": uv_prevday,
        "uv_prev": uv_prev,
        "uv_yoy": uv_yoy,
        "uv_ld_pct": pct_change(uv_today, uv_prevday),
        ...
        "orders_prevday": ord_prevday,
        "orders_prev": ord_prev,
        "orders_yoy": ord_yoy,
        "orders_ld_pct": pct_change(ord_today, ord_prevday),
        ...
        # p(percentage point) 변화: 카드에서 쓰는 값
        "cvr_ld_ppt": round(cvr_today - cvr_prevday, 1),
        "cvr_lw_ppt": round(cvr_today - cvr_prev, 1),
        "cvr_ly_ppt": round(cvr_today - cvr_yoy, 1),

        # --------------------
        # AOV
        # --------------------
        "aov_today": aov_today,
        "aov_prevday": aov_prevday,
        "aov_prev": aov_prev,
        "aov_yoy": aov_yoy,
        "aov_ld_pct": pct_change(aov_today, aov_prevday),
        ...
        # --------------------
        "new_today": new_today,
        "new_prevday": new_prevday,
        "new_prev": new_prev,
        "new_yoy": new_yoy,
        "new_ld_pct": pct_change(new_today, new_prevday),
        "new_lw_pct": pct_change(new_today, new_prev),
        "new_ly_pct": pct_change(new_today, new_yoy),

        # --------------------
        # Organic / Non-organic UV KPI
        # --------------------
        "organic_uv_today": organic_today,
        "organic_uv_prev": organic_prev,
        "organic_uv_yoy": organic_yoy,
        "organic_uv_lw_pct": pct_change(organic_today, organic_prev),
        "organic_uv_ly_pct": pct_change(organic_today, organic_yoy),
        "organic_uv_share_today": round(organic_share_today, 1),
        "organic_uv_share_prev": round(organic_share_prev, 1),
        "organic_uv_share_yoy": round(organic_share_yoy, 1),
        "organic_uv_share_lw_ppt": round(organic_share_today - organic_share_prev, 1),
        "organic_uv_share_ly_ppt": round(organic_share_today - organic_share_yoy, 1),

        "nonorganic_uv_today": nonorganic_today,
        "nonorganic_uv_prev": nonorganic_prev,
        "nonorganic_uv_yoy": nonorganic_yoy,
        "nonorganic_uv_lw_pct": pct_change(nonorganic_today, nonorganic_prev),
        "nonorganic_uv_ly_pct": pct_change(nonorganic_today, nonorganic_yoy),
        "nonorganic_uv_share_today": round(nonorganic_share_today, 1),
    }
    return kpi
...
def build_signals(kpi, funnel_rate_df, traffic_df, search_df):
    ...
    if kpi["revenue_lw_pct"] > 0 and kpi["cvr_lw_pct"] > 0:
        signals.append(
            f"· 매출이 전주 동일 요일 대비 {kpi['revenue_lw_pct']:.1f}% ↑, CVR은 {kpi['cvr_lw_pct']:.1f}% 개선되었습니다."
        )
        ...
        signals.append(
            f"· 매출 {kpi['revenue_lw_pct']:.1f}%, UV {kpi['uv_lw_pct']:.1f}%, CVR {kpi['cvr_lw_pct']:.1f}% 변동을 보였습니다."
        )
  ...
  <tr>
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {format_money_manwon(kpi['revenue_prev'])} · LY: {format_money_manwon(kpi['revenue_yoy'])}
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#eef2ff; color:#374151; margin-right:4px;">
            L.D {kpi['revenue_ld_pct']:+.1f}%
          </span>
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['uv_prev']:,}명 · LY: {kpi['uv_yoy']:,}명
        </div>
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          Organic: {kpi['organic_uv_today']:,}명 ({kpi['organic_uv_share_today']:.1f}% of UV)
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#eef2ff; color:#374151; margin-right:4px;">
            L.D {kpi['uv_ld_pct']:+.1f}%
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
            LW {kpi['uv_lw_pct']:+.1f}%
          </span>
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['cvr_prev']:.2f}% · LY: {kpi['cvr_yoy']:.2f}%
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#eef2ff; color:#374151; margin-right:4px;">
            L.D {kpi['cvr_ld_ppt']:+.1f}p
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
            LW {kpi['cvr_lw_ppt']:+.1f}p
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#fdeaea; color:#c53030;">
            LY {kpi['cvr_ly_ppt']:+.1f}p
          </span>
        </div>
      </div>
    </td>
  ...
  <tr>
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['orders_prev']:,}건 · LY: {kpi['orders_yoy']:,}건
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#eef2ff; color:#374151; margin-right:4px;">
            L.D {kpi['orders_ld_pct']:+.1f}%
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {format_money(kpi['aov_prev'])} · LY: {format_money(kpi['aov_yoy'])}
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#eef2ff; color:#374151; margin-right:4px;">
            L.D {kpi['aov_ld_pct']:+.1f}%
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['new_prev']:,}명 · LY: {kpi['new_yoy']:,}명
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#eef2ff; color:#374151; margin-right:4px;">
            L.D {kpi['new_ld_pct']:+.1f}%
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
  ...
  <tr>
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['organic_uv_prev']:,}명 · LY: {kpi['organic_uv_yoy']:,}명
        </div>
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          전체 UV 중 {kpi['organic_uv_share_today']:.1f}%
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['nonorganic_uv_prev']:,}명 · LY: {kpi['nonorganic_uv_yoy']:,}명
        </div>
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          전체 UV 중 {kpi['nonorganic_uv_share_today']:.1f}%
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
    ...
    <td width="33.3%" valign="top">
      <div style="background:#ffffff; border-radius:16px; padding:14px 16px; border:1px solid #e1e7f5;">
        ...
        <div style="font-size:18px; font-weight:700; margin-bottom:4px;">
          {kpi['organic_uv_share_today']:.1f}%
        </div>
        <div style="font-size:10px; color:#999; margin-bottom:4px;">
          LW: {kpi['organic_uv_share_prev']:.1f}% · LY: {kpi['organic_uv_share_yoy']:.1f}%
        </div>
        <div>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#e7f5ec; color:#1b7f4d; margin-right:4px;">
            LW {kpi['organic_uv_share_lw_ppt']:+.1f}p
          </span>
          <span style="display:inline-block; font-size:10px; padding:2px 7px; border-radius:999px; background:#fdeaea; color:#c53030;">
            LY {kpi['organic_uv_share_ly_ppt']:+.1f}p
          </span>
        </div>
      </div>
    </td>
